...
electron_1.ipcMain.handle('snapshot:createBatch', async (_event, snapshots) => {
    try {
        const prisma = await (0, database_1.getPrismaClient)();
        // Créer tous les snapshots en une transaction
        const createdSnapshots = await prisma.$transaction(snapshots.map((snapshot) => prisma.snapshot.create({
            data: {
...
electron_1.ipcMain.handle('snapshot:create', async (_event, data) => {
    try {
        const prisma = await (0, database_1.getPrismaClient)();
...
electron_1.ipcMain.handle('snapshot:getByAsset', async (_event, assetId) => {
    try {
        const prisma = await (0, database_1.getPrismaClient)();
...
electron_1.ipcMain.handle('snapshot:getLatest', async () => {
    try {
        const prisma = await (0, database_1.getPrismaClient)();
        ...
        const latestSnapshots = await Promise.all(assets.map(async (asset) => {
...
electron_1.ipcMain.handle('snapshot:getTotalValue', async (_event, date) => {
    try {
        const prisma = await (0, database_1.getPrismaClient)();
...
electron_1.ipcMain.handle('snapshot:getHistory', async () => {
    try {
        const prisma = await (0, database_1.getPrismaClient)();
        const snapshots = await prisma.snapshot.findMany({
            include: { asset: true },
